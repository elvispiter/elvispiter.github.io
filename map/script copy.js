ymaps.ready(function () {
  ...
  // Create the List
var options = {
  valueNames: [ 'address', 'side', 'id' ],
  item: `<a class="list__item" href="javascript:void(0)">
  <div class="address">Definately Maybe</div>
  <div class="tags meta">
      <div class="side tag">Oasis</div>
...
function jsonPreprocessor(data){
  let sides = [];
  for(let board of data){
    for(let index = 0; index < board.sides.length; index++){
      let side = Object()
      side.side = 'Сторона ' + board.sides[index]
...
fetch('https://elvispiter.github.io/elvis.json').then((r) => {
  r.json().then((json) => {
    boards = new List('results', options, jsonPreprocessor(json));
    boards.sort("id", {
      order: "desc"
    })
    var $count = $('.count')
    $count.append(boards.size());
    boards.on('filterComplete', function(){
      $count.text(boards.update().matchingItems.length);
    });
